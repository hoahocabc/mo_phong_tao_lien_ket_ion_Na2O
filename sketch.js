...
function draw() {
  ...
  if (state === "animating") {
    ...
    if (progress > 1) {
      ...
      // Thiết lập các electron này để di chuyển độc lập
      transferringElectrons[0].isTransferring = true;
      transferringElectrons[1].isTransferring = true;
    ...
    if (transferProgress > 1) {
      ...
      // Chuyển electron 1 vào lớp vỏ O
      atoms[1].shells[1].push({
          angle: PI,
          col: transferringElectrons[0].col,
          initialAngle: PI,
          targetAngle: PI
      });
      
      // Chuyển electron 2 vào lớp vỏ O
      atoms[1].shells[1].push({
          angle: 0,
          col: transferringElectrons[1].col,
          initialAngle: 0,
          targetAngle: 0
      });
    ...
    for (let i = 0; i < shellCounts.length; i++) {
      ...
      for (let j = 0; j < shellCounts[i]; j++) {
        shellElectrons.push({
          ...
          isTransferring: false // Thêm trạng thái để kiểm soát việc chuyển động
        });
      }
    ...
    for (let i = 0; i < this.shells.length; i++) {
      if (this.shells[i].length > 0) {
        ...
        for (let e of this.shells[i]) {
          // Chỉ vẽ các electron không đang trong quá trình chuyển
          if (!e.isTransferring) {
            let angle;
            let dynamicSpeed = 0.02;
